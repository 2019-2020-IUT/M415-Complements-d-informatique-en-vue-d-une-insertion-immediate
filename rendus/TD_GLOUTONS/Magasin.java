...
public class Magasin {
    ...
    private int volerAux(int poidsMax, int j) {
    	...
    	if(j >= 0) {
    		p = magasin[j].getPoids();
    		v = magasin[j].getValeur();
    		
    		if(poidsMax < p) {
    			return volerAux(poidsMax, j - 1);
    		} else {
    			int avec = v + volerAux(poidsMax - p, j - 1);
    			int sans = volerAux(poidsMax, j - 1);
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        Magasin m6 = new Magasin();
        System.out.println("Sac de poids max 30000: " + m6.voler(30000));
        System.out.println();
